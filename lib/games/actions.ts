...
const TITLE_MODEL: GameModelId = "claude-haiku-4-5"

/**
 * Names a game after the prompt it was created from.
 *
 * This runs before the composer can navigate anywhere, so it uses a cheap,
 * fast model and falls back to the raw prompt if the model is slow,
...
/**
 * Creates a game from the composer prompt and navigates to it.
 *
 ...
 * URL — see the redirect below. Every game lands in the same shared workspace,
 * which is the whole point of a demo that needs no accounts.
 */
export async function createGame(prompt: string, modelId: GameModelId) {
  ...
  // Checked rather than trusted: this action is reachable by direct POST, and
  // the value goes straight into the URL below.
